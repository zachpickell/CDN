...
// Stream an incoming web ReadableStream straight to disk, so multi-GB uploads
// never have to sit in memory. Returns the metadata entry (with byte size).
export async function saveStream({ originalName, mime, webStream }) {
  await ensureDirs();
  ...
  try {
    ...
    // Clean up the partial file if the upload failed or was aborted.
